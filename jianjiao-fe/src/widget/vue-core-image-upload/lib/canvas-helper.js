  ...
  **/
  crop(image, options, callback) {
    ...
    if(checkNumber(options.toCropImgX) && checkNumber(options.toCropImgY) && options.toCropImgW > 0 && options.toCropImgH > 0) {
      ...
      ctx.drawImage(image, options.toCropImgX, options.toCropImgY, options.toCropImgW, options.toCropImgH, 0 , 0, w, h);
      let mimeType = this._getImageType(image.src);
      let data = cvs.toDataURL(mimeType, options.compress/100);
      callback(data);
    }
  },
